...
The following example shows how to use neo4j plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run neo4j Service
        run: fluentci run --wasm neo4j start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm neo4j start
    displayName: start neo4j server
`}
</CodeBlock>
</TabItem>
