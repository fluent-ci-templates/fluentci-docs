...
The following example shows how to use confd plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run confd Service
        run: fluentci run --wasm confd start
`}
    </CodeBlock>
  </TabItem>
...
confd:
  extends: .fluentci
  script: - fluentci run --wasm confd start
`}
...
jobs:
  job:
    steps:
      - checkout
      - run: |
          sudo apt-get update && sudo apt-get install -y curl unzssip
...
steps:
  ...
  - script: fluentci run --wasm confd start
    displayName: start confd service
`}
</CodeBlock>
</TabItem>
