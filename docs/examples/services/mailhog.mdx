...
The following example shows how to use mailhog plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run mailhog Service
        run: fluentci run --wasm mailhog start
`}
    </CodeBlock>
  </TabItem>
...
mailhog:
  extends: .fluentci
  script: - fluentci run --wasm mailhog start
`}
...
steps:
  ...
  - script: fluentci run --wasm mailhog start
    displayName: start mailhog server
`}
</CodeBlock>
</TabItem>
