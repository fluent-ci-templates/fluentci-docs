...
The following example shows how to use tidb plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run tidb Service
        run: fluentci run --wasm tidb start
`}
    </CodeBlock>
  </TabItem>
...
tidb:
  extends: .fluentci
  script: - fluentci run --wasm tidb start
`}
