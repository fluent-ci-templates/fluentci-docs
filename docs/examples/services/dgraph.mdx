...
The following example shows how to use dgraph plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run dgraph Service
        run: fluentci run --wasm dgraph start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm dgraph start
    displayName: start dgraph server
`}
</CodeBlock>
</TabItem>
