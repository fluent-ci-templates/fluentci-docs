...
The following example shows how to use envconsul plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run envconsul Service
        run: fluentci run --wasm envconsul start
`}
    </CodeBlock>
  </TabItem>
...
envconsul:
  extends: .fluentci
  script: - fluentci run --wasm envconsul start
`}
...
steps:
  ...
  - script: fluentci run --wasm envconsul start
    displayName: start envconsul service
`}
</CodeBlock>
</TabItem>
