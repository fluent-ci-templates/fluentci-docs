...
The following example shows how to use redis plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run redis Service
        run: fluentci run --wasm redis start
`}
    </CodeBlock>
  </TabItem>
...
redis:
  extends: .fluentci
  script: - fluentci run --wasm redis start
`}
...
steps:
  ...
  - script: fluentci run --wasm redis start
    displayName: start redis server
`}
</CodeBlock>
</TabItem>
