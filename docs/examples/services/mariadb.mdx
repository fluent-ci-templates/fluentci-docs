...
The following example shows how to use mariadb plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run mariadb Service
        run: fluentci run --wasm mariadb start
`}
    </CodeBlock>
  </TabItem>
...
mariadb:
  extends: .fluentci
  script: - fluentci run --wasm mariadb start
`}
...
steps:
  ...
  - script: fluentci run --wasm mariadb start
    displayName: start mariadb server
`}
</CodeBlock>
</TabItem>
