...
The following example shows how to use mailcatcher plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run mailcatcher Service
        run: fluentci run --wasm mailcatcher start
`}
    </CodeBlock>
  </TabItem>
...
mailcatcher:
  extends: .fluentci
  script: - fluentci run --wasm mailcatcher start
`}
...
steps:
  ...
  - script: fluentci run --wasm mailcatcher start
    displayName: start mailcatcher server
`}
</CodeBlock>
</TabItem>
