...
The following example shows how to use minio plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run minio Service
        run: fluentci run --wasm minio start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm minio start
    displayName: start minio server
`}
</CodeBlock>
</TabItem>
