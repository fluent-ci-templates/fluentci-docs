...
The following example shows how to use mailpit plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run mailpit Service
        run: fluentci run --wasm mailpit start
`}
    </CodeBlock>
  </TabItem>
...
mailpit:
  extends: .fluentci
  script: - fluentci run --wasm mailpit start
`}
...
steps:
  ...
  - script: fluentci run --wasm mailpit start
    displayName: start mailpit server
`}
</CodeBlock>
</TabItem>
