...
The following example shows how to use spicedb plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run spicedb Service
        run: fluentci run --wasm spicedb start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm spicedb start
    displayName: start spicedb server
`}
</CodeBlock>
</TabItem>
