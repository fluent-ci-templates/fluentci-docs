...
The following example shows how to use memcached plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run memcached Service
        run: fluentci run --wasm memcached start
`}
    </CodeBlock>
  </TabItem>
...
memcached:
  extends: .fluentci
  script: - fluentci run --wasm memcached start
`}
...
steps:
  ...
  - script: fluentci run --wasm memcached start
    displayName: start memcached server
`}
</CodeBlock>
</TabItem>
