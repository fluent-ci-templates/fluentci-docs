...
The following example shows how to use rabbitmq plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run rabbitmq Service
        run: fluentci run --wasm rabbitmq start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm rabbitmq start
