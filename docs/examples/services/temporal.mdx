...
The following example shows how to use temporal plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run temporal Service
        run: fluentci run --wasm temporal start
`}
    </CodeBlock>
  </TabItem>
...
temporal:
  extends: .fluentci
  script: - fluentci run --wasm temporal start
`}
...
steps:
  ...
  - script: fluentci run --wasm temporal start
    displayName: start temporal server
`}
</CodeBlock>
</TabItem>
