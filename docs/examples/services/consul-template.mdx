...
The following example shows how to use consul-template plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run consul-template Service
        run: fluentci run --wasm consul-template start
`}
    </CodeBlock>
  </TabItem>
...
consul-template:
  extends: .fluentci
  script: - fluentci run --wasm consul-template start
`}
...
steps:
  ...
  - script: fluentci run --wasm consul-template start
    displayName: start consul-template service
`}
</CodeBlock>
</TabItem>
