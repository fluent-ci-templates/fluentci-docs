...
The following example shows how to use minikube plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run minikube Service
        run: fluentci run --wasm minikube start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm minikube start
    displayName: start minikube service
`}
</CodeBlock>
</TabItem>
