...
The following example shows how to use apache plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run Apache Service
        run: fluentci run --wasm apache start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm apache start
    displayName: start apache server
`}
</CodeBlock>
</TabItem>
