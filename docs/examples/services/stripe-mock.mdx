...
The following example shows how to use stripe-mock plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run stripe-mock Service
        run: fluentci run --wasm stripe-mock start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm stripe-mock start
    displayName: start stripe-mock server
`}
</CodeBlock>
</TabItem>
