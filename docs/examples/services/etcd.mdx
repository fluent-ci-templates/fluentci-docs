...
The following example shows how to use etcd plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run etcd Service
        run: fluentci run --wasm etcd start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm etcd start
    displayName: start etcd server
`}
</CodeBlock>
</TabItem>
