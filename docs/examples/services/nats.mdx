...
The following example shows how to use nats plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run nats Service
        run: fluentci run --wasm nats start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm nats start
    displayName: start nats server
`}
</CodeBlock>
</TabItem>
