...
The following example shows how to use nginx plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run nginx Service
        run: fluentci run --wasm nginx start
`}
    </CodeBlock>
  </TabItem>
...
nginx:
  extends: .fluentci
  script: - fluentci run --wasm nginx start
`}
...
steps:
  ...
  - script: fluentci run --wasm nginx start
    displayName: start nginx server
`}
</CodeBlock>
</TabItem>
