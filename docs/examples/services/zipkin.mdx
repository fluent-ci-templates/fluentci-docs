...
The following example shows how to use zipkin plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run zipkin Service
        run: fluentci run --wasm zipkin start
`}
    </CodeBlock>
  </TabItem>
...
zipkin:
  extends: .fluentci
  script: - fluentci run --wasm zipkin start
`}
...
steps:
  ...
  - script: fluentci run --wasm zipkin start
    displayName: start zipkin server
`}
</CodeBlock>
</TabItem>
