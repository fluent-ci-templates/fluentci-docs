...
The following example shows how to use dynamodb-local plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run dynamodb-local Service
        run: fluentci run --wasm dynamodb-local start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm dynamodb-local start
    displayName: start dynamodb-local server
`}
</CodeBlock>
</TabItem>
