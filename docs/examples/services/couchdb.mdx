...
The following example shows how to use couchdb plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run couchdb Service
        run: fluentci run --wasm couchdb start
`}
    </CodeBlock>
  </TabItem>
...
couchdb:
  extends: .fluentci
  script: - fluentci run --wasm couchdb start
`}
...
steps:
  ...
  - script: fluentci run --wasm couchdb start
    displayName: start couchdb server
`}
</CodeBlock>
</TabItem>
