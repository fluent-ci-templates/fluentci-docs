...
The following example shows how to use nsq plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Setup Fluent CI
        uses: fluentci-io/setup-fluentci@v5
      - name: Run nsq Service
        run: fluentci run --wasm nsq start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm nsq start
    displayName: start nsq server
`}
</CodeBlock>
</TabItem>
