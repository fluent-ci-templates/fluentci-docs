...
The following example shows how to use cockroachdb plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run cockroachdb Service
        run: fluentci run --wasm cockroachdb start
`}
    </CodeBlock>
  </TabItem>
...
cockroachdb:
  extends: .fluentci
  script: - fluentci run --wasm cockroachdb start
`}
...
steps:
  ...
  - script: fluentci run --wasm cockroachdb start
    displayName: start cockroachdb server
`}
</CodeBlock>
</TabItem>
