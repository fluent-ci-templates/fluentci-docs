...
The following example shows how to use quickwit plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run quickwit Service
        run: fluentci run --wasm quickwit start
`}
    </CodeBlock>
  </TabItem>
...
quickwit:
  extends: .fluentci
  script: - fluentci run --wasm quickwit start
`}
...
steps:
  ...
  - script: fluentci run --wasm quickwit start
    displayName: start quickwit server
`}
</CodeBlock>
</TabItem>
