...
The following example shows how to use postgres plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run postgres Service
        run: fluentci run --wasm postgres start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm postgres start
