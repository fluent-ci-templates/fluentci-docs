...
The following example shows how to use consul plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run consul Service
        run: fluentci run --wasm consul start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm consul start
    displayName: start consul server
`}
</CodeBlock>
</TabItem>
