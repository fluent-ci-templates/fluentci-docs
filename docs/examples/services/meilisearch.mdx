...
The following example shows how to use meilisearch plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run meilisearch Service
        run: fluentci run --wasm meilisearch start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm meilisearch start
    displayName: start meilisearch server
`}
</CodeBlock>
</TabItem>
