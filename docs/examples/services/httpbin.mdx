...
The following example shows how to use httpbin plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run httpbin Service
        run: fluentci run --wasm httpbin start
`}
    </CodeBlock>
  </TabItem>
...
httpbin:
  extends: .fluentci
  script: - fluentci run --wasm httpbin start
`}
...
steps:
  ...
  - script: fluentci run --wasm httpbin start
    displayName: start httpbin server
`}
</CodeBlock>
</TabItem>
