...
The following example shows how to use clickhouse plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run clickhouse Service
        run: fluentci run --wasm clickhouse start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm clickhouse start
    displayName: start clickhouse server
`}
</CodeBlock>
</TabItem>
