...
The following example shows how to use influxdb plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run influxdb Service
        run: fluentci run --wasm influxdb start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm influxdb start
    displayName: start influxdb server
`}
</CodeBlock>
</TabItem>
