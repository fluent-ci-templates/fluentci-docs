...
The following example shows how to use caddy plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run caddy Service
        run: fluentci run --wasm caddy start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm caddy start
    displayName: start caddy server
`}
</CodeBlock>
</TabItem>
