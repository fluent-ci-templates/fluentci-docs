...
The following example shows how to use vault plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run vault Service
        run: fluentci run --wasm vault start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm vault start
    displayName: start vault server
`}
</CodeBlock>
</TabItem>
