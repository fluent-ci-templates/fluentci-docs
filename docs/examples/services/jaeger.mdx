...
The following example shows how to use jaeger plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run jaeger Service
        run: fluentci run --wasm jaeger start
`}
    </CodeBlock>
  </TabItem>
...
jaeger:
  extends: .fluentci
  script: - fluentci run --wasm jaeger start
`}
...
steps:
  ...
  - script: fluentci run --wasm jaeger start
    displayName: start jaeger server
`}
</CodeBlock>
</TabItem>
