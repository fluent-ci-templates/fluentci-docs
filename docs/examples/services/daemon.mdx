...
The following example shows how to use daemon plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run daemon Service
        run: fluentci run --wasm daemon start demo pkgx bunx serve -p 4000
`}
    </CodeBlock>
  </TabItem>
...
daemon:
  extends: .fluentci
  script:
    - fluentci run --wasm daemon start demo pkgx bunx serve -p 4000
`}
...
steps:
  ...
  - script: fluentci run --wasm daemon start demo pkgx bunx serve -p 4000
    displayName: start daemon service
`}
</CodeBlock>
</TabItem>
