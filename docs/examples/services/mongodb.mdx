...
The following example shows how to use mongo plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run mongo Service
        run: fluentci run --wasm mongo start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm mongo start
    displayName: start mongo server
`}
</CodeBlock>
</TabItem>
