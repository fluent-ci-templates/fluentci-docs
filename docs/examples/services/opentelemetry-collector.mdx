...
The following example shows how to use opentelemetry-collector plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run opentelemetry-collector Service
        run: fluentci run --wasm opentelemetry-collector start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm opentelemetry-collector start
    displayName: start opentelemetry-collector server
`}
</CodeBlock>
</TabItem>
