...
The following example shows how to use arangodb plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run arangodb Service
        run: fluentci run --wasm arangodb start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm arangodb start
    displayName: start arangodb server
`}
</CodeBlock>
</TabItem>
