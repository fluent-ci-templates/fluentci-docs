...
The following example shows how to use php-fpm plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run php-fpm Service
        run: fluentci run --wasm php-fpm start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm php-fpm start
    displayName: start php-fpm server
`}
</CodeBlock>
</TabItem>
