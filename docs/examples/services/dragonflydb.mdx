...
The following example shows how to use dragonflydb plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run dragonflydb Service
        run: fluentci run --wasm dragonflydb start
`}
    </CodeBlock>
  </TabItem>
...
dragonflydb:
  extends: .fluentci
  script: - fluentci run --wasm dragonflydb start
`}
...
steps:
  ...
  - script: fluentci run --wasm dragonflydb start
    displayName: start dragonflydb server
`}
</CodeBlock>
</TabItem>
