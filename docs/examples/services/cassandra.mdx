...
The following example shows how to use cassandra plugin in different CI/CD Platforms:
  ...
  tasks:
    ...
    steps:
      ...
      - name: Run cassandra Service
        run: fluentci run --wasm cassandra start
`}
    </CodeBlock>
  </TabItem>
...
cassandra:
  extends: .fluentci
  script: - fluentci run --wasm cassandra start
`}
...
steps:
  ...
  - script: fluentci run --wasm cassandra start
    displayName: start cassandra server
`}
</CodeBlock>
</TabItem>
