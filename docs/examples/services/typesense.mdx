...
The following example shows how to use typesense plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run typesense Service
        run: fluentci run --wasm typesense start
`}
    </CodeBlock>
  </TabItem>
...
typesense:
  extends: .fluentci
  script: - fluentci run --wasm typesense start
`}
...
steps:
  ...
  - script: fluentci run --wasm typesense start
    displayName: start typesense server
`}
</CodeBlock>
</TabItem>
