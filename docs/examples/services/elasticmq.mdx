...
The following example shows how to use elasticmq plugin in different CI/CD Platforms:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Run elasticmq Service
        run: fluentci run --wasm elasticmq start
`}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm elasticmq start
    displayName: start elasticmq server
`}
</CodeBlock>
</TabItem>
