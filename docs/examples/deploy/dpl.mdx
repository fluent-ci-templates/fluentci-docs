...
[Dpl](https://github.com/travis-ci/dpl) (dee-pee-ell) is a deploy tool made for continuous deployment.
...
dag().call("https://pkg.fluentci.io/dpl@v0.1.0?wasm=1", "setup", vec![])?;
```

## CI/CD Integration

The following example shows how to integrate FluentCI with popular CI providers to deploy your project with Dpl:
      ...
      - name: Setup FluentCI
        ...
        with:
          ...
          args: |
            setup
      - name: deploy
        run: flox activate -- dpl heroku api --app $HEROKU_APP_NAME --api_key $HEROKU_API_KEY
        env:
...
deploy:
  ...
  script:
    - fluentci run --wasm dpl setup
    - flox activate -- dpl heroku api --app $HEROKU_APP_NAME --api_key $HEROKU_API_KEY
`}
