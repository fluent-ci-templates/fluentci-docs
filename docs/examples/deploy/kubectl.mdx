...
[Kubectl](https://kubernetes.io/docs/reference/kubectl/) is the Kubernetes command-line tool that allows you to run commands against Kubernetes clusters.
...
dag().call(
  "https://pkg.fluentci.io/kubectl@v0.1.0?wasm=1",
  "setup",
  vec!["latest"],
)?;
```
...
The following example shows how to integrate FluentCI with popular CI providers to setup Kubectl:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: Remove preinstalled kubectl
        run: |
          sudo rm -rf \`which kubectl\`
      - name: Setup Fluent CI
        uses: fluentci-io/setup-fluentci@v5
      - name: Install kubectl
        run: fluentci run --wasm kubectl setup
    `}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm kubectl setup
