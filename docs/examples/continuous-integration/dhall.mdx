...
[Dhall](https://github.com/dhall-lang/dhall-lang) is a programmable configuration language optimized for maintainability.
...
| Name   | Description                                                                |
| ------ | -------------------------------------------------------------------------- |
| setup  | Install a specific version of Dhall.                                       |
...
lint:
  extends: .fluentci
  script:
    - fluentci run --wasm dhall format
    - fluentci run --wasm dhall lint
`}
