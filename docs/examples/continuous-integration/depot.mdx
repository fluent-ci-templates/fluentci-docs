...
Provides the [Depot CLI](https://github.com/depot/cli) in your CI/CD pipeline.
...
| Name  | Description                          |
| ----- | ------------------------------------ |
| setup | Install a specific version of depot. |
| build | Start a build                        |
...
The following example shows how to integrate FluentCI with popular CI providers to set up Depot:
  ...
  job:
    steps:
      ...
      - run: |
          ...
          fluentci --version
        name: Setup FluentCI
      - run: fluentci run --wasm depot setup
        name: install depot
    machine:
