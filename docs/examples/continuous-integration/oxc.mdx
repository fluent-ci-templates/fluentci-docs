...
[Oxc](https://oxc.rs/) is s collection of JavaScript tools written in Rust.
...
| Name  | Description                      |
| ----- | -------------------------------- |
| setup | Install oxc tools                |
| lint  | Lint the project using oxc tools |
...
dag().call("https://pkg.fluentci.io/oxc@v0.1.0?wasm=1", "lint", vec![])?;
```

## CI/CD Integration

The following example shows how to integrate FluentCI with popular CI providers to lint a project using Oxc:
  ...
  job:
    steps:
      ...
      - run: |
          ...
          fluentci --version
        name: Setup FluentCI
      - run: fluentci run --wasm oxc lint
        name: install oxc
    machine:
