...
```bash
fluentci run --wasm java setup zulu-17.46.19
```
...
| Name  | Description                         |
| ----- | ----------------------------------- |
| setup | Install a specific version of Java. |
...
The following example shows how to integrate FluentCI with popular CI providers to install Java:
  ...
  job:
    steps:
      ...
      - run: |
          ...
          fluentci --version
        name: Setup FluentCI
      - run: fluentci run --wasm java setup zulu-17.46.19
        name: install java
    machine:
