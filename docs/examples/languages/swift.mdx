...
```bash
fluentci run --wasm swift test
fluentci run --wasm swift build
```
...
dag().call(
  "https://pkg.fluentci.io/swift@v0.7.0?wasm=1",
  "test",
  vec![],
);
dag().call(
...
The following examples show how to integrate FluentCI with popular CI providers to build and test Swift projects:
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: test
        run: fluentci run --wasm swift test
      - name: build
        run: fluentci run --wasm swift build
        `}
    </CodeBlock>
  </TabItem>
...
steps:
  ...
  - script: fluentci run --wasm swift test
    displayName: test
  - script: fluentci run --wasm swift build
    displayName: build
    `}
</CodeBlock>
</TabItem>
