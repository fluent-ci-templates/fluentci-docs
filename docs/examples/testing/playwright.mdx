...
| help         | display help for Playwright commands                                           |
| version      | display version of Playwright                                                  |
...
dag().call(
  "https://pkg.fluentci.io/playwright@v0.1.0?wasm=1",
  "install",
  vec![],
);
...
tests:
  ...
  script:
    - fluentci run --wasm playwright install 
    - fluentci run --wasm playwright  run
`}
