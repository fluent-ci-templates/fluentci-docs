...
fluentci run --wasm flutter build apk --release apk --release
```
...
| Name         | Description                    |
| ------------ | ------------------------------ |
| setup        | Install Flutter SDK            |
| code_quality | Run static code analysis       |
| test         | Run test-suites                |
| build        | Compile and bundle the project |
...
dag().call(
  "https://pkg.fluentci.io/flutter@v0.8.0?wasm=1",
  "code_quality",
  vec![],
);
...
jobs:
  tasks:
    ...
    steps:
      ...
      - name: build
        run: fluentci run --wasm flutter build apk --release`}
    </CodeBlock>
  </TabItem>
...
build:
  extends: .fluentci
  script: 
    - fluentci run --wasm flutter build apk --release
`}
...
steps:
  ...
  - script: fluentci run --wasm flutter test
    displayName: test
  - script: fluentci run --wasm flutter build apk --release
    displayName: build
`}
</CodeBlock>
</TabItem>
